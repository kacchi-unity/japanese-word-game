...
어려웠던 점: 가로 2행 스크롤뷰(Grid Layout Group + Content Size Fitter)를 구현하는 과정에서, 특정 카드 버튼들의 중앙 영역을 마우스 오버하거나 클릭할 때 하이라이트가 끊기거나 클릭 입력이 무시되는 기괴한 '도넛 형태의 사각지대'가 발생함.

현상 분석: 카드 내부의 앵커(Anchor) 위치를 수정하고 그리드 레이아웃의 셀 사이즈(Cell Size)를 프리팹 실제 크기와 일치시켰음에도 불구하고, 화면 중앙을 가로지르는 "정중앙 가로 일자 선(영역)"이 통째로 먹통이 되어 위/아래 카드의 경계선 클릭을 완벽하게 방패막이처럼 차단하고 있었음.

원인 분석 (집요한 EventSystem 추적):
"컴퓨터는 거짓말을 하지 않는다"는 원칙 하에, 런타임 중 EventSystem 오브젝트의 Pointer Enter 상태를 실시간 모니터링하며 디버깅을 진행함. 조사 결과, 범인은 카드 프리팹 내부에 배치된 특정 텍스트 라벨(TMP "Sword Record Text Label")이었음.

해당 텍스트 오브젝트의 크기(Rect Transform)가 의도치 않게 위아래로 길게 늘어나 있었고, UI 캔버스 상에서 투명한 상태로 화면 중앙의 넓은 영역을 덮어버려 자식 버튼으로 가야 할 마우스 레이캐스트를 본인이 대신 흡수(Blocking)하고 있었던 것이 원인이었음.

해결: 오브젝트 레이캐스트 최적화 및 타겟 단일화

UI 클릭의 무결성을 보장하기 위해, 클릭 이벤트를 직접 수신해야 하는 최상위 부모 컴포넌트(Body 프리팹의 Button)를 제외한 내부의 모든 자식 텍스트(TMP) 및 이미지 컴포넌트의 Raycast Target 옵션을 전수 조사하여 전면 체크 해제(OFF)함.

이를 통해 눈에 보이지 않는 투명한 글자 상자가 마우스 입력을 훔쳐 가던 장벽을 완전히 제거하였고, 레이아웃 구조를 해치지 않으면서 버튼 인식이 정확하게 작동하도록 구조를 정상화함.
