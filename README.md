...
- 게임 클리어 시 Result 씬 내 자동 저장 기능 추가

## 6/12
- 빌드를 위한 가변 데이터 1차 구조 개편 (로비 세팅)

## 6/13
- 가변 데이터 2차 구조 개편 (Game Session, EP 보유량)

## 6/16
- 가변 데이터 3차 구조 개편 (전체 단어 데이터)

- 가변 데이터 4차 구조 개편 (획득 단어 도감 데이터)

- 빌드 테스트 완료

## 6/17
- 도감 내 단어 별 정답 및 오답 횟수 UI 출력 기능 추가 (마우스 오버 시)
...
- 문제점: 유니티 에디터에서는 SO를 자유롭게 접근해 런타임에 데이터를 가변 데이터 처럼 다룰 수 있어 그대로 SO에 가변 데이터를 추가하여 구조를 설계함. 그러나 실제 게임 빌드 버전에선 ScriptableObject(SO) 내부의 가변 데이터가 저장되지 않고 씬을 이동할 때마다 SO의 초기 설정 값으로 돌아와 가변 데이터가 손상되는 치명적인 결함을 발견함. 가변 데이터와 불변 데이터의 분리된 처리 방식이 필요함을 알아냄. 

- [1차 해결]: 이를 해결하기 위해 1차적으로 GameDataManager를 만듦. 싱글톤으로 만들어 런타임 내 접근 가능하도록 설계하고, 해당 스크립트에 로컬 JSON 세이브, 로드 방식을 도입함. 실제 LobbySettingSO의 가변 데이터와 불변 데이터를 클래스로 나누어, GameDataManager를 통해 인스펙터로 직접 연결하여 가변 데이터의 저장 구조를 개편했음.
그러나 이 구조는 로비 세팅 SO 단 하나만을 타겟으로 하드코딩되어 있어 확장성에 심각한 한계가 있었음. 추후 GameSessionSO 데이터, SwordRecordSO 단어 도감 데이터 등 가변 데이터를 가진 SO를 새로 추가해야 함. 이때 GameDataManager에 각각 고유 SO의 저장, 불러오기, 초기화 코드를  매번 새로 짜고 수정해야 하는 구조적 결함(유지보수성 저하)이 존재했음.

- [2차 해결]: 새로운 가변 SO 데이터가 추가되어도 인스펙터 연결 외에 GameDataManager 코드를 단 한 줄도 건드리지 않도록 인터페이스, 상위 추상 상속 클래스 기반의 제네릭 구조로 재 개편함.

* ISaveSO 인터페이스 정의
: GameDataManager에서 세이브, 로드, 대상 SO 이름에 접근할 수 있도록 표준 규격을 강제함.

* BaseSaveSO<T> 추상 부모 클래스 구축
: 실제 자식 SO내 가변 데이터 클래스(RuntimeData)를 제네릭 <T>로 품을 수 있도록 설계하고 이 클래스를 대상으로 직접 세이브, 로드를 처리하는 공통 구조를 설계함.
: 초기화(Initialize)메서드는 SO마다 통일하기 어려워, 추상 메서드로 선언하여 자식 메서드에서 직접 초기화를 Override 메서드로 작성해야하는 구조를 설계함.
: ISaveSO를 상속받아 향후 추가될 모든 가변 SO가 이를 상속받아 규격을 통일하도록 개선하여 구조 통일성을 확보함.

* GameDataManager 제네릭 캐싱 시스템 전환
: 매니저가 개별 SO 변수를 직접 가지는 대신, 인스펙터에 등록된 SO들을 Dictionary<Type, ISaveSO>에 자동으로 담아 일괄 관리하도록 만듦.
: BaseSaveSO를 통해 각각의 가변 데이터 처리 대상 SO에게 세이브, 로드, 초기화 기능을 이전함.
: 외부 스크립트 사용 시 메서드인 GetData<T>()를 통해 원하는 SO 데이터 인스턴스를 런타임 내 동적으로 안전하게 꺼내 쓰도록 통로를 개설함.

이런 구조 설계를 통하여 가변 데이터로 다뤄야 하는 SO는
1. BaseSaveSO<T> 상속, 런타임 데이터 클래스 전달(T), 초기화(Initialize) 메서드 오버라이드 작성
2. GameDataManager 내 SO 인스펙터 연결

두 과정만 거치면 알아서 가변 데이터로 인식하고 처리하는 구조를 개편할 수 있게됨.
해당 개편 과정 학습을 통해 불변 데이터는 SO에, 가변 데이터는 SO 저장이 아닌 로컬 json으로 런타임에 따로 저장해야 한다는 필요성을 깨달음.
...
## 추후 개선 사항
- JSON을 활용해 실제 JLPT 단어 호출 (구현 완료)

- JLPT 급수 별 레벨 분류

- 게임 재미를 위한 Player의 스킬 추가

- Scene을 통해 게임 난이도 세분화 (구현 완료)

- 난이도 조절 시트템 구현 (구현 완료)

- 학습 단어 수집 사전 시스템 구현 (구현 완료)
